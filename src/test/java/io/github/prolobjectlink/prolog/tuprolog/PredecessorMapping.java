...
public class PredecessorMapping implements Prologable<Predecessor> {

	@Override
	public Predecessor fromTerm(PrologProvider provider, PrologTerm t) {
		String name = t.getArgument(0).getFunctor();
		String child = t.getArgument(1).getFunctor();
		Predecessor Predecessor = new Predecessor(name, child);
		return Predecessor;
	}

	@Override
	public PrologTerm toTerm(PrologProvider provider, Object o) {
		if (o instanceof Predecessor) {
			Predecessor p = (Predecessor) o;
			...
			if (name == null && child != null) {
				return provider.newStructure("predecessor", x, child);
			}
			if (name != null && child == null) {
				return provider.newStructure("predecessor", name, y);
			}
			if (name == null && child == null) {
				return provider.newStructure("predecessor", x, y);
			}
			return provider.newStructure("predecessor", name, child);
		}
	...
	public PrologTerm toTerm(PrologProvider provider) {
		...
		return provider.newStructure("predecessor", arguments);
	}

	@Override
	public Class<Predecessor> getType() {
