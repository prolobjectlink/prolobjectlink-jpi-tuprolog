...
public class TuPrologParameter extends TuPrologVariable implements PrologParameter {
	...
	TuPrologParameter(PrologProvider provider, PrologTerm name) {
		super(PrologTermType.PARAMETER_TYPE, provider);
		this.value = new Var(((PrologVariable) name).getName());
	}

	TuPrologParameter(PrologProvider provider, int position) {
		super(PrologTermType.PARAMETER_TYPE, provider, new Var());
	}

	TuPrologParameter(PrologProvider provider, String name, int position) {
		super(PrologTermType.PARAMETER_TYPE, provider, new Var(name));
	}
